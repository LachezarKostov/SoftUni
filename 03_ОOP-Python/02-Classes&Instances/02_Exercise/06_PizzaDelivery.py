...
class PizzaDelivery:
    ...
    def pizza_ordered(self) -> str:
        if self.ordered:
            return f"Pizza {self.name} already prepared and we can't make any changes!"
        self.ordered = True
        str_of_ingredients = ", ".join([f"{i}: {q}" for i, q in self.ingredients.items()])
        return f"You've ordered pizza {self.name} prepared with {str_of_ingredients} and the price will be {self.price}lv."
